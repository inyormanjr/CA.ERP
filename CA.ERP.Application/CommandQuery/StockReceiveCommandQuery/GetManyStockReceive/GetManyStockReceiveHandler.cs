...
namespace CA.ERP.Application.CommandQuery.StockReceiveCommandQuery.GetManyStockReceive
{
    public class GetManyStockReceiveHandler : IRequestHandler<GetManyStockReceiveQuery, PaginatedResponse<StockReceiveView>>
    {
        ...
        {
            List<StockReceive> stockReceives = await _stockReceiveRepository.GetManyStockReceiveAsync(request.Branch, request.SupplierId, request.DateCreated, request.DateReceived, request.Source, request.Stage, request.Skip, request.Take, cancellationToken);
            int totalCount = await _stockReceiveRepository.GetManyStockReceiveCountAsync(request.Branch, request.SupplierId, request.DateCreated, request.DateReceived, request.Source, request.Stage, cancellationToken);
