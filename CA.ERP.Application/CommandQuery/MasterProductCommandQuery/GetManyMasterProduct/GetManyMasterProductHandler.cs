...
namespace CA.ERP.Application.CommandQuery.MasterProductCommandQuery.GetManyMasterProduct
{
    public class GetManyMasterProductHandler : IRequestHandler<GetManyMasterProductQuery, PaginatedResponse<MasterProductView>>
    {
        ...
        {
            var masterProducts = await _masterProductRepository.GetManyAsync(request.Model, request.BrandId, request.Skip, request.Take, request.Status, cancellationToken: cancellationToken);
            var count = await _masterProductRepository.GetCountAsync(request.Model, request.BrandId, request.Status, cancellationToken: cancellationToken);
